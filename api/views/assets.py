...
def get_assets(request, *args, **kwargs):
    ...
    기간 윈도우: 최근 30일 이내 생성분만(2026-09-25, 90일→30일로 변경). 한때
    STAGE=상따 등 "결과 난 것"은 오래돼도 예외로 계속 보여줬었는데, 마이그레이션한
    실서버 데이터 중 오래전에 상따까지 간 건들이 수십 건씩 계속 떠서 사용자가
    다시 뺌 — 이제 그냥 기간으로만 자름, 예외 없음."""
