...
--
-- 2026-09-04: assets.js의 18개 shed endpoint 코드를 컬럼 단위로 전수 대조해서
-- 재설계함(원본 prod 컬럼을 그대로 복제하지 않고, 우리가 포팅하는 범위에서
-- 실제로 읽거나 쓰는 컬럼만 남김):
--   제거 — TEACHER_SABUN/TEACHER_NAME/IS_FLAGGED/STRATEGY_LINK/TOOL/
--         SHEET_CONFIG_ID/SNAP_AREA_ID: 18개 endpoint 어디서도 0회 사용
--         (SHEET_CONFIG_ID는 prod 실 데이터도 1802건 전부 NULL로 확인됨).
--   수정 — GUIDE_SABUN: cancel-shed-habjaeyang이 `GUIDE_SABUN = NULL` UPDATE를
--         실행함(인도자 취소 플로우) — NOT NULL이면 즉시 제약 위반. prod의
--         실제 제약도 이미 nullable(체크인된 DDL만 stale). nullable로 둠.
--
...
COMMENT ON TABLE  PROSPECTS            IS 'Shed 리드 메인 테이블. dev DB, TEAMS FK 없음, 파티션 없음. 18개 shed endpoint에서 실사용하는 컬럼만 유지';
COMMENT ON COLUMN PROSPECTS.TEAM_ID    IS 'FK 없는 평문 팀 식별자 (USERS.TEAM_ID와 동일 규칙)';
COMMENT ON COLUMN PROSPECTS.GUIDE_SABUN IS 'nullable — cancel-shed-habjaeyang이 인도자 취소 시 NULL로 되돌림';
